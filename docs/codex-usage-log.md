...
- 브라우저 대본 설계: 실시간 RAF 위에서 30Hz 권위 tick에 입력을 정확한 tick으로 맞출 수 없으므로, 먼저 권위 시뮬레이션에서 "사람이 실제로 칠 수 있는" 대본만 후보로 두고 timing 변형에 견디는지 측정했다. `w→d→s→a` 2초 kiting + HUD 피로 55% 교대는 hold 45~75틱 x gap 0~8틱의 45개 변형 전부에서 seed 47 승리, 같은 kiting에서 교대만 빼면 21개 변형 전부 패배였다. 무입력은 강화 카드 클릭 tick이 `awaiting-upgrade`에서 얼어 있어 실시간 편차와 무관하게 결정론적이므로(seed 47 → `패배/두 분대가 모두 쓰러졌습니다./처치 50/구조 0/생존 16`) 결과 통계 정확 일치는 이 경로에 두었다. 구조 완료는 구조자 피격 1회당 진행도 -15 때문에 일반 플레이에서 성립하지 않음을 확인해(45틱 창에 도달 못 함) 브라우저 테스트는 실제 lock과 `#N 구조 중 k/45` HUD 표시·해제까지만 단언하고, 완료 구조 증거는 기존 `runRescueAgencyScenario`에 남겼다.
